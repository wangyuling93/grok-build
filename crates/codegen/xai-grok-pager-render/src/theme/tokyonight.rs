...
impl Theme {
    ...
    /// rows, the queue pane, the dock). Half-blend of the design canvas toward
    /// `bg_dark`, softer than the dropdown `bg_hover` token. Named ANSI and Reset
    /// palettes cannot blend, so fall back to `bg_hover` (Reset on the terminal
    /// theme: no band). Transparent mode still blends against the design RGB.
    pub fn row_hover_bg(&self) -> Color {
        self.blend_canvas(self.bg_dark, 0.5).unwrap_or(self.bg_hover)
    }
