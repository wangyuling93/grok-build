...
pub fn render_key() -> RenderKey {
    RenderKey {
        ...
        terminal_native: terminal_native_active(),
    }
}
...
pub fn terminal_native_active() -> bool {
    terminal_native_locked()
        || ThemeKind::decode(CURRENT.load(Ordering::Relaxed)).is_terminal_native()
}
...
pub fn reset_for_test() {
    // Tests are serialized via TEST_LOCK so the AtomicU32/AtomicBool pair is safe to reset without any cross-thread coordination
