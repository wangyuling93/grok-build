...
///
/// Delegates to [`ThemeKind::is_dark`] so new light themes stay correct without hard-coding a kind list.
/// The render worker maps this to `xai_grok_mermaid::MermaidTheme`; it lives here (rather than referencing the engine crate) so the always-compiled detection module stays independent of the optional `mermaid` feature.
