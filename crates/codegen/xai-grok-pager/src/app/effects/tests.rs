...
/// Persistence tickets follow effect-issue order even when the runtime first
/// polls their tasks in reverse order.
#[tokio::test]
async fn transparency_persist_tickets_preserve_request_order() {
    {
    ...
    let mut first = transparency_persist_ticket(false, 1);
    let mut second = transparency_persist_ticket(true, 2);
    let mut third = transparency_persist_ticket(false, 3);
    let order = Arc::new(std::sync::Mutex::new(Vec::new()));

    let third_order = Arc::clone(&order);
    let third_handle = tokio::spawn(async move {
        third.wait_for_turn().await;
    ...
    // A fresh AppView rebases the confirmed value, then successful and failed
    // mappings carry the exact value/generation and roll back to the last
    // confirmed disk value rather than the effect-local optimistic fallback.
    let mut fresh_view = transparency_persist_ticket(false, 1);
    fresh_view.wait_for_turn().await;
    assert!(!confirmed_transparency_value(true));
    drop(fresh_view);

    let success = route_transparency_persist_result(Ok(()), true, false, 41);
    assert!(matches!(
        success,
    ...
    assert!(confirmed_transparency_value(false));

    let failure = route_transparency_persist_result(Err("disk full".into()), false, false, 42);
    assert!(matches!(
        failure,
    ...
    let mut pending = transparency_persist_ticket(true, 43);
    let (release_tx, release_rx) = tokio::sync::oneshot::channel::<()>();
    let pending_handle = tokio::spawn(async move {
        pending.wait_for_turn().await;
        let _ = release_rx.await;
    });
    tokio::task::yield_now().await;
    let flush_handle = tokio::spawn(flush_transparency_persistence());
    tokio::task::yield_now().await;
    assert!(
        !flush_handle.is_finished(),
        "shutdown flush returned before the current tail completed"
    );
    release_tx.send(()).unwrap();
    pending_handle.await.unwrap();
