            ...
            msg = async {
                ...
                while drained < ACP_DRAIN_BATCH_MAX && input_rx.is_empty() {
                    ...
                    if !app.pending_effects.is_empty() {
                        let effs = std::mem::take(&mut app.pending_effects);
                        if process_effects(effs, &mut tasks, &mut app, &progress_tx) {
                            return Ok(finish_run_with_stall_flush(&mut app, &mut stall_rollup).await);
                        }
                    }
                }
...
async fn finish_run_with_stall_flush(
    app: &mut AppView,
    stall_rollup: &mut super::event_loop_stall::StallRollup,
) -> RunResult {
    flush_pending_stall(stall_rollup);
    finish_run(app).await
}

/// Exit funnel: releases the startup obligation, flushes transparent-background
/// persistence, shuts down notifications, and builds the exit summary.
...
mod tests {
    ...
    fn finish_run_unused_home_session_has_no_exit_info() {
        ...
        assert!(matches!(app.active_view, ActiveView::Welcome));
        assert!(
            make_run_result(&app).exit_info.is_none(),
