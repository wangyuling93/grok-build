...
impl PromptWidget {
    ...
    pub fn draw(
            ...
            && ta_area.height > 0
        {
            ...
            let interim_style = match theme.blend(bg, theme.text_secondary, 0.7) {
        ...
        // canvas). Skip when the bg is overridden — the prompt is inline elsewhere.
    ...
    fn chrome_caption_style(bg: ratatui::style::Color, theme: &Theme, focused: bool) -> Style {
        ...
        match theme.blend(bg, theme.text_secondary, opacity) {
            Some(fg) => Style::default().fg(fg).bg(bg),
        ...
        for flag in info.flags {
            ...
            let mut style = if let Some(color) = flag.color {
                if flag.bold {
                    ...
                    let dimmed = theme.blend(bg, color, flag_opacity).unwrap_or(color);
                ...
                match theme.blend(bg, theme.gray, flag_opacity) {
