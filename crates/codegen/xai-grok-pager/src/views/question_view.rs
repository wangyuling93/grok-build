...
fn apply_row_overlay(line: &mut Line<'_>, overlay: Style) {
...
fn apply_hover_overlay(line: &mut Line<'_>, theme: &Theme, background: ratatui::style::Color) {
    apply_row_overlay(line, theme.hover_overlay_style(background));
}
...
pub fn build_flat_option_lines(
    ...
    for (i, option) in question.options.iter().enumerate() {
        ...
        if embed.is_none() {
            if is_cursor_item && panel_focused {
                let ov = theme.selection_overlay_style(row_bg, true);
                for line in &mut all_lines[start..] {
                    apply_row_overlay(line, ov);
                }
...
fn build_freeform_line(
    ...
    let overlay = if embed.is_none() && is_cursor && panel_focused {
        Some(theme.selection_overlay_style(row_bg, true))
    } else if embed.is_none() && is_hovered {
        Some(theme.hover_overlay_style(row_bg))
    } else {
        None
    };
